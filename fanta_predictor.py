...
def run(prov, roster, season, now, check_only=False):
    ...
    for r in roster.itertuples():
        ...
        rows.append({
            ...
            "Fantavoto": round(pj["fv"], 2), "Voto": round(pj["rating"], 1), "EV": round(pj["ev"], 2), "Rif": round(refs[role], 3),
...
def to_html(df, fixtures, now, modules):
    d = df.copy()
...
def main(argv=None):
    ...
    (out / "index.html").write_text(to_html(df.reset_index(drop=True), fixtures, now,
                                            [m.strip() for m in a.moduli.split(",")]), encoding="utf-8")
